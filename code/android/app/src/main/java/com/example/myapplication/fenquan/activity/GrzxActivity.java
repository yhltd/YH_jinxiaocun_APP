...
import com.example.myapplication.fenquan.service.GeRenService;
...
public class GrzxActivity extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.fenquan_grzx);

        MyApplication myApplication = (MyApplication) getApplication();
        renyuan = myApplication.getRenyuan();

        companyLogo = findViewById(R.id.company_logo);
        geRenService = new com.example.myapplication.fenquan.service.GeRenService(this);
        setupAvatarClick();
        loadPushNewsData();
        ...
        String sjh = renyuan.getPhone();
        String yx = renyuan.getEmail();
        String zhuangtai = renyuan.getZhuangtai();
        String company = renyuan.getB();
        String Bianhao = renyuan.getBianhao();
        String yonghuming = renyuan.getC();

        // 在界面显示用户信息
        TextView tvSjh = findViewById(R.id.sjh);
        TextView tvCompany = findViewById(R.id.ssgs);
        TextView tvYonghuming = findViewById(R.id.yonghuming);
        TextView tvZhuangtai = findViewById(R.id.zhzt);
        TextView tvBianhao = findViewById(R.id.ygbh);
        TextView tvYx = findViewById(R.id.yx);
        ...
        tvYx.setText(yx);
        tvYonghuming.setText(yonghuming);
    ...
    private void setWelcomeText() {
        ...
        if (companyName != null && !companyName.isEmpty()) {
            ...
            welcomeMessage = "欢迎使用" + shortCompanyName + "分权系统";
        } else {
            // 如果缓存中没有公司名称，使用默认文本
            welcomeMessage = "欢迎使用云合未来分权系统";
        }
    ...
    private void loadPushNewsData() {
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    // 获取数据
                    List<com.example.myapplication.fenquan.entity.GeRen> result = geRenService.getList();

                    // 处理返回数据
                    if (result != null && !result.isEmpty()) {
                        com.example.myapplication.fenquan.entity.GeRen news = result.get(0);
    ...
    private void setupAvatarClick() {
        ...
        companyLogo.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                Toast.makeText(GrzxActivity.this, "点击头像可以更换", Toast.LENGTH_SHORT).show();
                return true;
            }
        });
    }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    // 构建更新SQL
                    String sql = "UPDATE baitaoquanxian_renyun SET touxiang = ? WHERE B = ? AND E = ? AND D = ?";
                    ...
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            if (success) {
                                Toast.makeText(GrzxActivity.this, "头像更新成功", Toast.LENGTH_SHORT).show();
                            } else {
                                Toast.makeText(GrzxActivity.this, "头像更新失败", Toast.LENGTH_SHORT).show();
                            }
                        }
                    });
                    ...
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(GrzxActivity.this, "上传失败: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                        }
                    });
                }
            }
    ...
    private boolean updateAvatarInDatabase(String sql, String base64Image, String company, String account, String password) {
        FenquanDao baseDao = new FenquanDao<>();
        try {
