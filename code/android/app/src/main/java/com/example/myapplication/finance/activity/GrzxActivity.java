...
import com.example.myapplication.finance.service.GenRenService;
...
public class GrzxActivity extends AppCompatActivity {
    ...
    private GenRenService genRenService;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        genRenService = new com.example.myapplication.finance.service.GenRenService(this);
    ...
    private void loadPushNewsData() {
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    // 获取数据
                    List<com.example.myapplication.finance.entity.GeRen> result = genRenService.getList();

                    // 处理返回数据
                    if (result != null && !result.isEmpty()) {
                        com.example.myapplication.finance.entity.GeRen news = result.get(0);
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    // 构建更新SQL
                    String sql = "UPDATE Account SET touxiang = ? WHERE company = ? AND name = ? AND pwd = ?";
    ...
    private boolean updateAvatarInDatabase(String sql, String base64Image, String company, String account, String password) {
        financeBaseDao baseDao = new financeBaseDao();
        try {
