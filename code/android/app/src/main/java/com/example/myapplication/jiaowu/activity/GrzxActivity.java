...
import com.example.myapplication.jiaowu.service.GeRenService;
import com.example.myapplication.jiaowu.service.PushNewsService;
...
public class GrzxActivity extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.jiaowu_grzx);

        MyApplication myApplication = (MyApplication) getApplication();
        teacher = myApplication.getTeacher();

        companyLogo = findViewById(R.id.company_logo);
        geRenService = new com.example.myapplication.jiaowu.service.GeRenService(this);
        setupAvatarClick();
        loadPushNewsData();


        String zhzt =  teacher.getState();
        String username =  teacher.getUsername();
        String company =  teacher.getCompany();
        String Bianhao =  teacher.getPhone();
        String Yuangongname =  teacher.getRealname();
    ...
    private void setWelcomeText() {
        ...
        if (companyName != null && !companyName.isEmpty()) {
            ...
            welcomeMessage = "欢迎使用" + shortCompanyName + "教务系统";
        } else {
            // 如果缓存中没有公司名称，使用默认文本
            welcomeMessage = "欢迎使用云合未来教务系统";
        }
    ...
    private void loadPushNewsData() {
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    // 获取数据
                    List<com.example.myapplication.jiaowu.entity.GeRen> result = geRenService.getList();

                    // 处理返回数据
                    if (result != null && !result.isEmpty()) {
                        com.example.myapplication.jiaowu.entity.GeRen news = result.get(0);
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    // 构建更新SQL
                    String sql = "UPDATE teacher SET touxiang = ? WHERE Company = ? AND UserName = ? AND Password = ?";
    ...
    private boolean updateAvatarInDatabase(String sql, String base64Image, String company, String account, String password) {
        JiaowuBaseDao baseDao = new JiaowuBaseDao<>();
        try {
