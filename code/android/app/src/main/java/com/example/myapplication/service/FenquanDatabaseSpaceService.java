...
public class FenquanDatabaseSpaceService {
    private static final String TAG = "FenquanDatabaseSpaceService";

    private FenquanDao<?> fenquanDao;

    public FenquanDatabaseSpaceService() {
        fenquanDao = new FenquanDao<>();
    }

    /**
     * 获取分权系统数据库中各表按公司统计的使用空间（SQL Server版本）
    ...
    public double getDatabaseSizeByCompany(String companyName) {
        ...
        tables.put("baitaoquanxian", "公司");
        tables.put("baitaoquanxian_copy1", "quanxian");
        tables.put("baitaoquanxian_copy2", "公司");
        tables.put("baitaoquanxian_department", "company");
        tables.put("baitaoquanxian_gongsi", "B");
        tables.put("baitaoquanxian_jisuan", "company");
        tables.put("baitaoquanxian_renyun", "B");

        try {
            // 通过反射获取 FenquanDao 中的静态连接
            ...
            try {
                ...
                // 注意：不要关闭 conn，因为它是 FenquanDao 共享的
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    private Connection getConnectionFromDao() {
        try {
            java.lang.reflect.Field field = FenquanDao.class.getDeclaredField("conn");
