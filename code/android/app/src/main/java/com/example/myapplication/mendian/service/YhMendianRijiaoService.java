...
public class YhMendianRijiaoService {
    ...
    public List<YhMendianRijiao> getList(String shou, String fu, String chi,String start_date,String end_date, String company) {
        String sql = "select * from day_trading right join customer on customer.id = day_trading.id where day_trading.gongsi = ? and recipient like '%' ? '%' and cardholder like '%' ? '%' and drawee like '%' ? '%' and date_time >= ? and date_time <= ?";
        base = new MendianDao();
