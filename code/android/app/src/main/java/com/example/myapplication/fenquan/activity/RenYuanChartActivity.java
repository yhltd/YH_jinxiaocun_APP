...
public class RenYuanChartActivity extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.renyuan_chart);
    ...
    private void initList() {
        ...
        nameText = name.getSelectedItem().toString();
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    workbenchService = new WorkbenchService();
                    list = workbenchService.getRenYuanChart(renyuan.getB(),nameText, start_dateText ,stop_dateText);
                    if(list == null){
    ...
    private void get_xiala() {
        Handler listLoadHandler = new Handler(new Handler.Callback() {
            @Override
            public boolean handleMessage(Message msg) {
                SpinnerAdapter adapter = new ArrayAdapter<String>(RenYuanChartActivity.this, android.R.layout.simple_spinner_dropdown_item, name_array);
                name.setAdapter(StringUtils.cast(adapter));
                return true;
            }
        });
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    workbenchService = new WorkbenchService();
                    List<Renyuan> this_list = renyuanService.getList(renyuan.getB(), "");
                    name_array = new ArrayList<>();
                    if (this_list.size() > 0) {
                        for (int i = 0; i < this_list.size(); i++) {
                            if(!this_list.get(i).getC().equals(""))
                                name_array.add(this_list.get(i).getC());
                        }
                    }
    ...
    protected void showDatePickDlg(final EditText editText) {
        ...
        DatePickerDialog datePickerDialog = new DatePickerDialog(RenYuanChartActivity.this, new DatePickerDialog.OnDateSetListener() {
