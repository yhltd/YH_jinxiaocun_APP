...
public class FinanceDatabaseSpaceService {
    private static final String TAG = "FinanceDatabaseSpaceService";

    private financeBaseDao<?> financeBaseDao;

    public FinanceDatabaseSpaceService() {
        financeBaseDao = new financeBaseDao<>();
    }

    /**
     * 获取财务系统数据库中各表按公司统计的使用空间（SQL Server版本）
    ...
    public double getDatabaseSizeByCompany(String companyName) {
        ...
        tables.put("Account", "company");
        tables.put("Accounting", "company");
        tables.put("Department", "company");
        tables.put("FinancingExpenditure", "company");
        tables.put("FinancingIncome", "company");
        tables.put("gongzimingxi", "company");
        tables.put("InvestmentExpenditure", "company");
        tables.put("InvestmentIncome", "company");
        tables.put("Invoice", "company");
        tables.put("InvoicePeizhi", "company");
        tables.put("KehuPeizhi", "company");
        tables.put("ManagementExpenditure", "company");
        tables.put("ManagementIncome", "company");
        tables.put("shuilvPeizhi", "company");
        tables.put("SimpleAccounting", "company");
        tables.put("SimpleData", "company");
        tables.put("VoucherSummary", "company");
        tables.put("VoucherWord", "company");
        tables.put("waibiPeizhi", "company");
        tables.put("ysyfpeizhi", "company");

        try {
            // 通过反射获取 financeBaseDao 中的静态连接
            ...
            try {
                ...
                // 注意：不要关闭 conn，因为它是 financeBaseDao 共享的
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    private Connection getConnectionFromDao() {
        try {
            java.lang.reflect.Field field = financeBaseDao.class.getDeclaredField("conn");
