            ...
            public void run() {
                ...
                try {
                    ...
                    for (int i = 0; i < list.size(); i++) {
                        if (list.get(i).getAdminis().equals("true")) {
                            list.get(i).setAdminis("管理员");
                        } else {
                            list.get(i).setAdminis("普通用户");
                        }
                        HashMap<String, Object> item = new HashMap<>();
                        item.put("name", list.get(i).getName());
                        item.put("password", list.get(i).getPassword());
                        item.put("adminis", list.get(i).getAdminis());
                        item.put("btype", list.get(i).getBtype());
                        data.add(item);
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                ...
                adapter = new SimpleAdapter(UserActivity.this, data, R.layout.user_row, new String[]{"name", "password", "adminis", "btype"}, new int[]{R.id.name, R.id.password, R.id.adminis, R.id.btype}) {
                    @Override
                ...
                adapter_block = new SimpleAdapter(UserActivity.this, data, R.layout.user_row_block, new String[]{"name", "password", "adminis", "btype"}, new int[]{R.id.name, R.id.password, R.id.adminis, R.id.btype}) {
                    @Override
                    public View getView(int position, View convertView, ViewGroup parent) {
                        final LinearLayout view = (LinearLayout) super.getView(position, convertView, parent);
                        LinearLayout linearLayout = (LinearLayout) view.getChildAt(0);
                ...
                builder.setPositiveButton("确定", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        new Thread(new Runnable() {
                            ...
                            public void run() {
                                Message msg = new Message();
                                msg.obj = yhJinXiaoCunUserService.delete(list.get(position).get_id());
