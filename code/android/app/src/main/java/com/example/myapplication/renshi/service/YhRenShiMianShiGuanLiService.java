...
public class YhRenShiMianShiGuanLiService {
     ...
     * 查询简历管理列表
     */
    public List<YhRenShiMianShiGuanLi> getList(String company, int page, int pageSize) {
        int offset = (page - 1) * pageSize;
        String sql = "select * from gongzi_jianliguanli where gongsi = ? and zhuangtai = '通过' ";

        base = new renshiBaseDao();
        return base.query(YhRenShiMianShiGuanLi.class, sql , company.replace("_hr",""));
    }

    /**
     * 根据状态筛选查询
     */
    public List<YhRenShiMianShiGuanLi> getListByStatus(String company, String status) {
        String sql;
        List<YhRenShiMianShiGuanLi> result;

        if (status == null || status.isEmpty() || status.equals("全部")) {
            // 查询所有记录
            sql = "select * from gongzi_jianliguanli where gongsi = ? and zhuangtai = '通过' order by id desc";
            base = new renshiBaseDao();
            result = base.query(YhRenShiMianShiGuanLi.class, sql, company.replace("_hr",""));
        } else {
            // 按状态筛选
            sql = "select * from gongzi_jianliguanli where gongsi = ? and zhuangtai = ? order by id desc";
            base = new renshiBaseDao();
            result = base.query(YhRenShiMianShiGuanLi.class, sql, company.replace("_hr",""), status);
        }
     ...
     * 新增简历记录
     */
    public boolean insert(YhRenShiMianShiGuanLi entity) {
        String sql = "insert into gongzi_jianliguanli (gongsi, touliren, xueli, mubiaogangwei, tijiaoshijian, beizhu, zhuangtai, mianshipizhu) " +
                "values(?, ?, ?, ?, ?, ?, ?, ?)";
        base = new renshiBaseDao();
        return base.execute(sql,
                ...
                entity.getBeizhu(),
                entity.getZhuangtai(),
                entity.getMianshipizhu());
    }

    /**
     * 更新简历记录
     */
    public boolean update(YhRenShiMianShiGuanLi entity) {
        String sql = "update gongzi_jianliguanli set touliren=?, xueli=?, mubiaogangwei=?, tijiaoshijian=?, beizhu=?, zhuangtai=?, mianshipizhu=? where id=?";
        base = new renshiBaseDao();
        return base.execute(sql,
                entity.getTouliren(),
                entity.getXueli(),
    ...
    public boolean canDelete(int id) {
        ...
        List<YhRenShiMianShiGuanLi> result = checkDao.query(YhRenShiMianShiGuanLi.class, checkSql, id);

        if (result != null && result.size() > 0) {
            YhRenShiMianShiGuanLi entity = result.get(0);
    ...
    public boolean canUpdateFiles(int id) {
        ...
        renshiBaseDao checkDao = new renshiBaseDao();
        List<YhRenShiMianShiGuanLi> result = checkDao.query(YhRenShiMianShiGuanLi.class, selectSql, id);
    ...
    public String getCurrentFilesadd(int id) {
        ...
        List<YhRenShiMianShiGuanLi> result = selectDao.query(YhRenShiMianShiGuanLi.class, selectSql, id);
    ...
    public String getCurrentFiles(int id) {
        ...
        List<YhRenShiMianShiGuanLi> result = base.query(YhRenShiMianShiGuanLi.class, selectSql, id);
